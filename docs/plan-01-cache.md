...
- **In:** `soc_claw/cache.py` module — `Cache` Protocol, `InMemoryCache`, `RedisCache`, factory. Unit + integration tests.
...
| Backend selection | `SOC_CLAW_REDIS_URL` env var | Empty → InMemory; set → Redis. Already documented in [config-reference.md](config-reference.md) |
...
2. Create `soc_claw/cache.py`:
   ...
   - `get_cache()` factory reading `SOC_CLAW_REDIS_URL`.
...
5. Add `RedisCache` class to `soc_claw/cache.py`, same Protocol. Use `SETEX` for TTL.
6. Update factory: `SOC_CLAW_REDIS_URL` set → return `RedisCache`.
...
- `from soc_claw.cache import get_cache` returns the right backend per env.
- TTLs respected for both backends.
- All tests pass; CI runs Redis tests against a service container.
- No public API changes to `soc_claw/tools/*` yet — that is cutover work.
