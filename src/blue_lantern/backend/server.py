"""Blue Lantern FastAPI Backend.

Serves the HTML UI from blue_lantern/frontend/templates/index.html and exposes
...
Authentication (S1):  session-cookie auth via ``blue_lantern.backend.auth``.
...
from blue_lantern.backend.security import build_csp_header, build_security_config
from blue_lantern.observability.logging_config import setup_logging
from blue_lantern.observability.telemetry import setup_tracing
...
from blue_lantern.backend.auth import (  # noqa: E402  (after observability bootstrap)
    SECRET_KEY,
    SESSION_COOKIE,
    SESSION_MAX_AGE,
    authenticate,
...
from blue_lantern.backend.routes.batch_api import router as batch_api_router

logger = logging.getLogger("blue-lantern.server")

app = FastAPI(title="Blue Lantern")

# Store environment variables in app state for access by routes
app.state.env = {
    "GCS_LOG_BUCKET_NAME": os.environ.get("GCS_LOG_BUCKET_NAME", ""),
    "BLUE_LANTERN_BATCH_SIZE": int(os.environ.get("BLUE_LANTERN_BATCH_SIZE", "30")),
    "BLUE_LANTERN_GCS_POLL_INTERVAL": int(os.environ.get("BLUE_LANTERN_GCS_POLL_INTERVAL", "300")),
}
...
async def startup_event():
    ...
    logger.info("Starting Blue Lantern backend")

    # Async Redis client for batch-job tracking. Read by routes via
    # request.app.state.redis; left as None when BLUE_LANTERN_REDIS_URL is
    ...
    # Start GCS poller
    try:
        from blue_lantern.connectors.gcs_poller import start_gcs_poller
    ...
    try:
        from blue_lantern.connectors.kafka_consumer import start_consumer
    ...
    try:
        from blue_lantern.connectors.dlq_reprocessor import start_dlq_reprocessor
...
async def shutdown_event():
    ...
    logger.info("Stopping Blue Lantern backend")

    # Stop GCS poller
    try:
        from blue_lantern.connectors.gcs_poller import stop_gcs_poller
    ...
    # Stop Kafka consumer
    try:
        from blue_lantern.connectors.kafka_consumer import stop_consumer
        await stop_consumer()
    ...
    try:
        from blue_lantern.connectors.dlq_reprocessor import stop_dlq_reprocessor
